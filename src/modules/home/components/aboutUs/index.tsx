    ...
    <div className="w-full element__container ">
      <div className="flex relative">
        <div className={classes.bgImage}>
          ...
          <div className=" absolute right-0 bottom-0 bg-white border-none py-5 px-2 inline-flex gap-1">
            {heros.cards.map((el:ArrayObjectsType, index:number) => (
              <div key={index} className=" flex">
                <span className=" text-fs-3 font-secondaryRegular text-secondary font-wt-700 leading-lh-3">{el.number}</span>
                <div className=" text-descp-color font-primaryRegular text-fs-1 font-wt-400 leading-6 break-after-all w-32 inline pr-12 pl-4  ">{el.descp}</div>
              </div>
            ))}
          </div>
        </div>
